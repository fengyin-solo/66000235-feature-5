...
### 心律失常检测
- 按采集场景（静息 / 运动）使用两套判定口径，用户在侧边栏选定后各条判定按对应上下限执行：
  - 静息口径：心动过速 HR > 100 BPM，心动过缓 HR < 60 BPM，RR 变异系数 CV > 0.15
  - 运动口径：心动过速 HR > 150 BPM，心动过缓 HR < 50 BPM，RR 变异系数 CV > 0.20
- 阈值边界（如静息 100 / 60 BPM）属于正常区间，严格越过才判异常，不会两边都判
- ST 段抬高检测（提示心肌梗死可能），与场景无关，始终执行
- 心律不规则检测（疑似房颤）
- 结论与事件描述均标注本次实际使用的口径（`[静息口径]` / `[运动口径]`）
- 同参数（导联、时长、采样率、心率、场景）重复提交结果与第一次完全一致（信号生成使用确定性随机种子）
- 置信度评分与中文诊断描述
